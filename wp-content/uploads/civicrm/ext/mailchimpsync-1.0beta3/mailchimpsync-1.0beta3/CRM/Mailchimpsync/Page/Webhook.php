...
class CRM_Mailchimpsync_Page_Webhook extends CRM_Core_Page {
  ...
  /**
   * Used to help identify log output.
   * @var string */
  protected $webhookId;
  ...
  public function run() {
    ...
    if (empty($_POST)) {
      ...
      Civi::log()->info("[mailchimpsync] Empty webhook received. Probably just a ping.");
      CRM_Utils_System::civiExit();
    }

    $this->webhookId = substr(sha1(serialize($_POST)), 0, 6);
    $this->logInfo('received : ' . json_encode($_POST));
    ...
    try {
      ...
      if (method_exists($this, $method)) {
        ...
        $this->logInfo("$method completed, response code: $response_code");
      }
      elseif ($method === 'processCampaign') {
        $this->logWarning("Webhook set to include Campaign events - this is not a good idea.");
        $response_code = 204;
      }
      else {
      ...
      $this->logNotice("Cannot sync: " . $e->getMessage());
      $response_code = 204;
    }
    catch (Exception $e) {
      $this->logError(get_class($e) . ": " . $e->getMessage(), ['exception' => $e]);
      $response_code = 400;
    }
  ...
  public function logInfo($message, $vars=NULL) {
    Civi::log()->info($this->prepareLogMessage($message, $vars));
  }
  public function logWarning($message, $vars=NULL) {
    Civi::log()->warning($this->prepareLogMessage($message, $vars));
  }
  public function logError($message, $vars=NULL) {
    Civi::log()->error($this->prepareLogMessage($message, $vars));
  }
  public function logNotice($message, $vars=NULL) {
    Civi::log()->notice($this->prepareLogMessage($message, $vars));
  }
  public function prepareLogMessage($message) {
    $message = "[mailchimpsync] webhook {$this->webhookId}: $message";
    return $message;
  }
}
